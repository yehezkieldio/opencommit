...
import {
  sliceToTokenLimit,
  splitToTokenChunks
} from './utils/slice-to-token-limit.js';
...
function splitDiffByFiles(diff: string, maxTokens: number): DiffChunk[] {
  ...
  for (const fileDiff of fileDiffs) {
    ...
    if (
      currentChunk.tokenCount + fileTokens > maxTokens &&
...
async function synthesizeCommitMessage(
  ...
  const maxSummaryTokens =
    MAX_TOKENS_INPUT - promptTokens - MAX_TOKENS_OUTPUT - ADJUSTMENT_FACTOR;
  ...
  if (tokenCount(combinedSummary) > maxSummaryTokens) {
    finalSummary = await recursivelyReduceSummaries(
      summaries,
      maxSummaryTokens,
      0
    );
...
async function recursivelyReduceSummaries(
  ...
  for (const summary of summaries) {
    ...
    if (
      currentBatchTokens + summaryTokens > maxTokens / 2 &&

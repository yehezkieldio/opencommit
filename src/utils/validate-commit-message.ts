...
export function validateCommitMessage(
  ...
  if (!HEADER_PATTERN.test(firstLine)) {
    errors.push(
      `First line is not a valid conventional commit header: "${firstLine}"`
    );
  }
  ...
  if (colonIndex > -1) {
    ...
    if (subject.length > maxSubjectLength) {
      errors.push(
        `Subject line exceeds ${maxSubjectLength} characters (${subject.length})`
      );
    }
  }
...
export function collapseMultipleHeaders(
  ...
  if (
    !validation.additionalHeaders ||
    validation.additionalHeaders.length === 0
  ) {
    return message;
  }
